...
public class UpgradesInventory {
    ...
    private void createItemsPane(Plot plot, Player player) {
        ...
        GuiItem plotSizeUpgrade = new GuiItem(new ItemBuilder(HeadManager.getHeadFromMinecraftValueUrl("c61b279e00f19d9f69a712f6560767cae0cfa02c83160d46c7f50c7526f6776e"))
                ...
                .setLore("&7Obecny poziom: #<447cfc>" + plotUpgrades.getLevel(PlotUpgrades.Type.PLOT_SIZE_UPGRADE), "&7Cena za kolejny poziom: #<447cfc>" + plotUpgrades.getCostForNextLevel(plot, PlotUpgrades.Type.PLOT_SIZE_UPGRADE), "&7Obecne wymiary działki: #<447cfc>" + getPlotSizes(plotUpgrades.getLevel(PlotUpgrades.Type.PLOT_SIZE_UPGRADE)), "", "&7Domyślnie rozmiar działki to 41x41, możesz", "&7ją powiększyć poprzez zakupienie ulepszenia.", "&7Każde ulepszenie dodaje 5 bloków w jedną stronę,", "&7a 6 ulepszenie (dostępne tylko dla rang premium) daje", "&7aż 10 bloków w jedną stronę!"),
                event -> {
                    event.setCancelled(true);
    ...
    private void buyUpgrade(Plot plot, Player player, PlotUpgrades plotUpgrades, PlotUpgrades.Type type) {
        ...
        if (!vault.has(player, cost)) {
            player.sendMessage(FormatUtils.formatMessage("#<447cfc>&l☁ &cNie masz wystarczająco pieniędzy. Potrzeba: " + cost + " pieniędzy!"));
            return;
        }
